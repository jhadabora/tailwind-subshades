...
describe('parseCuloriRgb', () => {
    ...
    test.for([
        ...
        'returns other format color %s -> (%f,%f,%f)',
        ([color, r, g, b]) => {
            const result = formula.parseCuloriRgb(color);
            assert.isDefined(result);
            expect(result.mode).toBe('rgb');
    ...
    test.for([['oklch(0.8716 0.47 140)'], ['oklch(0.6245 0.2152 250)']])(
        'returns a valid color for out of rgb bounds color %s',
        ([color]) => {
            const result = formula.parseCuloriRgb(color);
            assert.isDefined(result);
            expect(result.mode).toBe('rgb');
...
describe('outputCuloriHex', () => {
    ...
    test.for([
        ...
        'returns rgb hex color with out of bounds rgb (%f,%f,%f) -> %s',
        ([r, g, b, hex]) => {
            const result = formula.outputCuloriHex({ mode: 'rgb', r, g, b });
            assert.isDefined(result);
            expect(result).toBe(hex);
...
describe('outputCuloriOklch', () => {
    ...
    test.for([
        ...
        'returns oklch color with out of bounds rgb (%f,%f,%f) -> oklch(%f,%f,%f)',
        ([r, g, b, l, c, h]) => {
            const result = formula.outputCuloriOklch({ mode: 'rgb', r, g, b });
            assert.isDefined(result);
            assert(result.startsWith('oklch('));
...
describe('createFormula', () => {
    test('runs all functions', () => {
        const parse = vi.fn((_color: string) => ({ mock: true }));
        const interpolate = vi.fn(
            (_color1: { mock: boolean }, _color2: { mock: boolean }, _weight: number) => ({
                mock2: true,
            }),
        );
        const output = vi.fn((_color: { mock2: boolean }) => 'mock3');
...
describe('v4rgbLerp', () => {
    test.for([
        ...
        'returns %s, %s, %f -> oklch(%f %f %f)',
        ([color1, color2, weight, l, c, h]) => {
