...
describe('outputCuloriHex', () => {
    ...
    test.for([
        ...
        [2.0, 0.0, 0.0, '#ff0000'],
        [0.0, 0.5, -2.0, '#008000'],
    ] as [number, number, number, string][])('returns rgb hex color with out of bounds rgb (%f,%f,%f) -> %s', ([
        r,
        g,
        b,
        hex,
...
describe('createFormula', () => {
    test('runs all functions', () => {
        ...
        const interpolate = vi.fn((_color1: { mock: boolean }, _color2: { mock: boolean }, _weight: number) => ({
            mock2: true,
        }));
...
describe('v4rgbLerp', () => {
    ...
    ] as [string, string, number, number, number, number][])('returns %s, %s, %f -> oklch(%f %f %f)', ([
        color1,
        color2,
        weight,
        l,
